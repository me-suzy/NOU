...
function viewimage(imagefile){
  open("imageload.php?image=" + imagefile+"&action=view",null,"left=300,top=200,status=yes,scrollbars=no,toolbar=no,menubar=no,location=no");
}
...
function getview(imagefile){
  this.opener.item_detailform.image_file.value=imagefile;
  this.opener.focus();
  this.close();
}
...
foreach ($result_array as $element){
  if (ereg("upload.php",$HTTP_REFERER){
    echo '<tr valign="top'>
          <td>
          <a name="'.basename($element).'" href="#" onClick="getrid("' .basename($element). '");">'.basename($element).'</a></td>
          <td align=right><a href="#" onClick="delimage("'.basename($element).'");">View It!</a></td></tr>\n';
  }
  else{
